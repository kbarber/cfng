/*
 * $Id: modes.c 744 2004-05-23 07:53:59Z skaar $
 *
...
void ParseModeString(char *modestring, mode_t *plusmask, mode_t *minusmask)
{
    ...
    /* Already found "sort" of mode */
    enum modesort found_sort = unknown;

    /* Sort of started but not yet finished mode */
    enum modesort sort = unknown;
    ...
    *plusmask = *minusmask = 0;
    ...
    for (sp = modestring; true ; *sp++) {
        ...
        case 'r':
            CheckModeState(which,state,symbolic,sort,*sp);
            value |= 0444 & affected;
            sort = symbolic;
            break;
        ...
        case '6':
        case '7':
            CheckModeState(which,state,numeric,sort,*sp);
            sort = numeric;
            gotaction = true;
            state = which;

            /* TODO: Hard-coded; see below */
            affected = 07777;

            sscanf(sp,"%o",&value);

            /* 
             * TODO: Hardcoded !  Is this correct for all sorts of Unix?
             * What about NT?  Any (POSIX)-constants??
             */
            if (value > 07777) {
        ...
        case '\0':
            if (state == who || value == 0) {
                if (strcmp(modestring,"0000") != 0 &&
                        strcmp(modestring,"000") != 0) {
        ...
        default:
            snprintf(g_output,CF_BUFSIZE,"Invalid mode string (%s)",
                        modestring);
            yyerror (g_output);
            break;
        }
...
void
CheckModeState(enum modestate stateA, enum modestate stateB,
        enum modesort sortA, enum modesort sortB, char ch)
{
    if ((stateA != wild) && (stateB != wild) && (stateA != stateB)) {
        sprintf(g_vbuff,"Mode string constant (%c) used out of context",ch);
        yyerror(g_vbuff);
    }
...
void
SetMask(char action,int value,int affected,mode_t *p,mode_t *m)
{
    ...
    case '=':
        ...
        *m |= ((~value) & 07777 & affected);
        return;
    default:
        sprintf(g_vbuff,"Mode directive %c is unknown",action);
        yyerror(g_vbuff);
        return;
    }
}
