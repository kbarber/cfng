/*
 * $Id: copy.c 734 2004-05-23 06:00:42Z skaar $
 *
...
/* 
 * Need a transparent way of getting this into CopyReg(). Use a public
 * member in struct Image.
 */

void
CheckForHoles(struct stat *sstat, struct Image *ip)
{
...
int
CopyRegDisk(char *source, char *new, struct Image *ip)
{
    int sd, dd, buf_size;
    char *buf, *cp;
    int n_read, *intp;
    ...
    if ((sd = open(source,O_RDONLY|O_BINARY)) == -1) {
        snprintf(g_output,CF_BUFSIZE,"Can't copy %s!\n",source);
    ...
    if ((dd = open(new,O_WRONLY|O_CREAT|O_TRUNC|O_EXCL|O_BINARY, 0600)) == -1) {
        snprintf(g_output, CF_BUFSIZE, "Copy %s:%s security - failed "
                "attempt to exploit a race? (Not copied)\n",ip->server,new);
        CfLog(cfinform,g_output,"open");
        close(sd);
        unlink(new);
        return false;
            ...
            if (cp > buf + n_read) {
                ...
                if (lseek (dd, (off_t) n_read, SEEK_CUR) < 0L) {
                    ...
                    snprintf(g_output,CF_BUFSIZE,
                            "Copy failed (no space?) while doing %s to %s\n",
                            source,new);

                    CfLog(cferror,g_output,"lseek");
                    free(buf);
                    unlink(new);
                    close(dd);
                    close(sd);
...
int
EmbeddedWrite(char *new, int dd, char *buf, struct Image *ip, int towrite,
        int *last_write_made_hole, int n_read)
{
    int *intp;
    char *cp;
    ...
    if (ip->makeholes) {
        ...
        /* 
         * If we found the sentinel, the whole input block was zero, and
         * we can make a hole.
         */
    ...
    if (intp == 0) {
        if (cf_full_write (dd,buf,towrite) < 0) {
            snprintf(g_output, CF_BUFSIZE*2, 
                    "Local disk write(%.256s) failed\n", new);
            CfLog(cferror,g_output,"write");
            g_conn->error = true;
            return false;
        }
