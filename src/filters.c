/*
 * $Id: filters.c 741 2004-05-23 06:55:46Z skaar $
 *
...
InstallFilter(char *filter)
{
    ...
    if (g_vfilterlisttop == NULL) {
        g_vfilterlist = ptr;
    } else {
        g_vfilterlisttop->next = ptr;
    }
    ...
    ptr->defines = NULL;
    ptr->elsedef = NULL;
    g_vfilterlisttop = ptr;
}

/* ----------------------------------------------------------------- */

void
InstallFilterTest(char *alias, char *type, char *data)
{
    struct Filter *fp;

    int     crit, i = -1;
    char    ebuff[CF_EXPANDSIZE], vbuff[CF_BUFSIZE];
    char    units='x', *sp;
    time_t  now;
    ...
    if (crit == NoFilter) {
        snprintf(g_output,CF_BUFSIZE,"Unknown filter criterion (%s)\n",type);
        yyerror(g_output);
    } else {
        for (fp = g_vfilterlist; fp != NULL; fp = fp->next) {
            if (strcmp(alias,fp->alias) == 0) {
                ...
                case filtertosize:
                    ...
                    if (i < 0)
                    {
                    yyerror("filter size attribute with silly value "
                ...
                case filterexecregex:
                    for (sp = ebuff+strlen(ebuff)-1; (*sp != '(') &&
                            (sp > ebuff); sp--) { }
...
CheckFilters(void)
{
    ...
    for (fp = g_vfilterlist; fp != NULL; fp = fp->next) {
        if (fp->criteria[filterresult] == NULL) {
            snprintf(g_output,CF_BUFSIZE*2,"No result specified for filter %s",
                    fp->alias);
        ...
        if ((fp->criteria[filterfromctime]!=NULL &&
                    fp->criteria[filtertoctime]==NULL) ||
                (fp->criteria[filterfromctime]==NULL &&
                    fp->criteria[filtertoctime]!=NULL)) {

            snprintf(g_output,CF_BUFSIZE*2,
                    "Incomplete ctime limit specification of filter %s",
                    fp->alias);
        ...
        if ((fp->criteria[filterfromatime]!=NULL &&
                    fp->criteria[filtertoatime]==NULL) ||
                (fp->criteria[filterfromatime]==NULL &&
                    fp->criteria[filtertoatime]!=NULL)) {

            snprintf(g_output,CF_BUFSIZE*2,
                    "Incomplete atime limit specification of filter %s",
                    fp->alias);
        ...
        if ((fp->criteria[filterfrommtime]!=NULL &&
                    fp->criteria[filtertomtime]==NULL) ||
                (fp->criteria[filterfrommtime]==NULL &&
                    fp->criteria[filtertomtime]!=NULL)) {

            snprintf(g_output,CF_BUFSIZE*2,
                    "Incomplete mtime limit specification of filter %s",
                    fp->alias);
        ...
        if ((fp->criteria[filterfromstime]!=NULL &&
                    fp->criteria[filtertostime]==NULL) ||
                (fp->criteria[filterfromstime]==NULL &&
                    fp->criteria[filtertostime]!=NULL)) {

            snprintf(g_output,CF_BUFSIZE*2,
                    "Incomplete stime limit specification of filter %s",
                    fp->alias);
        ...
        if ((fp->criteria[filterfromttime]!=NULL &&
                    fp->criteria[filtertottime]==NULL) ||
                (fp->criteria[filterfromttime]==NULL &&
                    fp->criteria[filtertottime]!=NULL)) {

            snprintf(g_output,CF_BUFSIZE*2,
                    "Incomplete ttime limit specification of filter %s",
                    fp->alias);
        ...
        if ((fp->criteria[filterfromsize]!=NULL &&
                    fp->criteria[filtertosize]==NULL) ||
                (fp->criteria[filterfromsize]==NULL &&
                    fp->criteria[filtertosize]!=NULL)) {

            snprintf(g_output,CF_BUFSIZE*2,
                    "Incomplete size limit specification of filter %s",
                    fp->alias);
        ...
        if (fp->criteria[filterfromatime] != NULL) {
            if (Date2Number(fp->criteria[filterfromatime],t) 
                    > Date2Number(fp->criteria[filtertoatime],t)) {

                snprintf(g_output,CF_BUFSIZE*2,
                        "To/From atimes silly in filter %s (from > to)",
                        fp->alias);
        ...
        if (fp->criteria[filterfromctime] != NULL) {
            if (Date2Number(fp->criteria[filterfromctime],t) 
                    > Date2Number(fp->criteria[filtertoctime],t)) {

                snprintf(g_output,CF_BUFSIZE*2,
                        "To/From ctimes silly in filter %s (from > to)",
                        fp->alias);
        ...
        if (fp->criteria[filterfrommtime] != NULL) {
            if (Date2Number(fp->criteria[filterfrommtime],t) 
                    > Date2Number(fp->criteria[filtertomtime],t)) {

                snprintf(g_output,CF_BUFSIZE*2,
                        "To/From mtimes silly in filter %s (from > to)",
                        fp->alias);
        ...
        if (fp->criteria[filterfromstime] != NULL) {
            if (Date2Number(fp->criteria[filterfromstime],t) 
                    > Date2Number(fp->criteria[filtertostime],t)) {

                snprintf(g_output,CF_BUFSIZE*2,
                        "To/From stimes silly in filter %s (from > to)",
                        fp->alias);
                CfLog(cferror,g_output,"");
                FatalError("Consistency errors");
            }
        ...
        if (fp->criteria[filterfromttime] != NULL) {
            if (Date2Number(fp->criteria[filterfromttime],t) 
                    > Date2Number(fp->criteria[filtertottime],t)) {

                snprintf(g_output,CF_BUFSIZE*2,
                        "To/From ttimes silly in filter %s (from > to)",
                        fp->alias);
                CfLog(cferror,g_output,"");
                FatalError("Consistency errors");
            }
            if (strncmp(fp->criteria[filterfromttime],
                        "accumulated",strlen("accumulated")) != 0) {
                yyerror("Must use accumulated time in FromTtime");
            }
            if (strncmp(fp->criteria[filtertottime],
                        "accumulated",strlen("accumulated")) != 0) {
        ...
        if (fp->criteria[filterfromsize] != NULL) {
            if (strcmp(fp->criteria[filterfromsize],
                        fp->criteria[filtertosize]) > 0) {
                snprintf(g_output,CF_BUFSIZE*2,
                        "To/From size is silly in filter %s (from > to)",
                        fp->alias);
                CfLog(cferror,g_output,"");
                FatalError("Consistency errors");
            }
...
enum filternames
FilterActionsToCode(char *filtertype)
{
    int i;
    ...
    for (i = 0; g_vfilternames[i] != '\0'; i++) {
        if (strcmp(g_vfilternames[i],filtertype) == 0) {
 ...
 */
time_t
Date2Number(char *string, time_t now)

{
    ...
    if (strcmp(string,"inf")==0) {
        /* time_t is signed long, assumes 64 bits */
        return (time_t) (now|0x0FFFFFFF);
    }

    sscanf(string,"%15[^(](%d,%d,%d,%d,%d,%d)",
            type,&year,&month,&day,&hr,&min,&sec);

    if (year*month*day*hr*min*sec < 0) {
        yyerror("Bad relative time specifier, should be "
                "type(year,month,day,hr,min,sec)");
        return now;
    }
    ...
    if (strcmp(type,"date")==0) {
        ...
        Debug("Total negative offset = %.1f minutes\n",
                (double)(g_cfstarttime-time)/60.0);
        Debug("Time computed from input was: %s\n",ctime(&time));
        return time;
    } else if (strcmp(type,"accumulated")==0) {
        sscanf(string,"%8[^(](%d,%d,%d,%d,%d,%d)",
                type,&year,&month,&day,&hr,&min,&sec);

        if (year*month*day*hr*min*sec < 0) {
            yyerror("Bad cumulative time specifier, should be "
                    "type(year,month,day,hr,min,sec)");
            return now;
        }
...
int
ProcessFilter(char *proc, struct Item *filterlist,
        char **names, int *start, int *end)
{
    ...
    for (fp = g_vfilterlist; fp != NULL; fp=fp->next) {
        if (IsItemIn(filterlist,fp->alias)) {
            ...
            if (fp->criteria[filterresult] == NULL) {
                fp->criteria[filterresult] = strdup("Owner.PID.PPID.PGID."
                        "RSize.VSize.Status.Command.TTime.STime."
                        "TTY.Priority.Threads");
            }
...
int
FileObjectFilter(char *file, struct stat *lstatptr,
        struct Item *filterlist, enum actions context)
{
    ...
    for (fp = g_vfilterlist; fp != NULL; fp=fp->next) {
        if (IsItemIn(filterlist,fp->alias)) {
            ...
            if (fp->criteria[filterresult] == NULL) {
                fp->criteria[filterresult] = strdup("Type.Owner.Group."
                        "Mode.Ctime.Mtime.Atime.Size.ExecRegex."
                        "NameRegex.IsSymLinkTo.ExecProgram");
            }
...
void
DoFilter(struct Item **attr, char **crit, struct stat *lstatptr, char *filename)
{
    ...
    if (crit[filterfromatime] != NULL) {
        if (FilterTimeMatch(lstatptr->st_atime,
                    crit[filterfromatime],crit[filtertoatime])) {
            PrependItem(attr,"Atime","");
        }
    }

    if (crit[filterfromctime] != NULL) {
    if (FilterTimeMatch(lstatptr->st_ctime,
                crit[filterfromctime],crit[filtertoctime])) {
            PrependItem(attr,"Ctime","");
        }
    }

    if (crit[filterfrommtime] != NULL) {
        if (FilterTimeMatch(lstatptr->st_mtime,
                    crit[filterfrommtime],crit[filtertomtime])) {
...
void
DoProc(struct Item **attr, char **crit, char **names, char **line)
{
    if (crit[filterowner] != NULL) {
    ...
    if (crit[filterfromttime] != NULL) {
        if (FilterProcTTimeMatch("TIME","TIME",crit[filterfromttime],
                    crit[filtertottime],names,line)) {
    ...
    if (FilterProcSTimeMatch("STIME","START",crit[filterfromstime],
                crit[filtertostime],names,line)) {
...
int
FilterTypeMatch(struct stat *lstatptr, char *crit)
{
...
int
FilterProcMatch(char *name1, char *name2, char *expr, char **names, char **line)
{
    int i;
    regex_t rx;
    regmatch_t pmatch;
...
int FilterProcSTimeMatch(char *name1, char *name2,
        char *fromexpr, char *toexpr, char **names, char **line)
{
    int i;
    time_t fromtime,totime, now = g_cfstarttime,pstime;
    char year[5],month[4],hr[3],min[3],day[3],timestr[256];

    memset(year,0,5);
    strcpy(year,g_vyear);
    memset(month,0,4);
    strcpy(month,g_vmonth);
    memset(day,0,3);
    strcpy(day,g_vday);
    memset(hr,0,3);
    strcpy(hr,g_vhr);
    memset(min,0,3);
    strcpy(min,g_vminute);
    ...
    for (i = 0; names[i] != NULL; i++) {
        if ((strcmp(names[i],name1) == 0) || (strcmp(names[i],name2) == 0)) {
            ...
            if (strstr(line[i],":")) {
                ...
                snprintf(timestr,256,"date(%s,%d,%s,%s,%s,0)",
                        year,Month2Number(month),day,hr,min);
            } else {
                /* date Month */
...
void
ParseTTime(char *line, char *time_str)
{
    ...
    if (strstr(line,":")) {
        if ((r = sscanf(line,"%d-%d:%d:%d",&day,&hr,&min,&sec)) == 4) {
            snprintf(time_str,256,"accumulated(0,0,%d,%d,%d,%d)",
                    day,hr,min,sec);
...
int
FilterProcTTimeMatch(char *name1, char *name2,
        char *fromexpr, char *toexpr, char **names, char **line)
{
    int i,hr=0,min=-1,sec=-1;
    time_t fromtime,totime,now = g_cfstarttime,pstime = g_cfstarttime;
    char timestr[256];
...
int
FilterOwnerMatch(struct stat *lstatptr, char *crit)
{
    struct Item *attrib = NULL;
    char buffer[64];
    struct passwd *pw;
...
int
FilterGroupMatch(struct stat *lstatptr, char *crit)
{
    struct Item *attrib = NULL;
    char buffer[64];
    struct group *gr;
...
int
FilterModeMatch(struct stat *lstatptr, char *crit)
{
    ...
    Debug("FilterModeMatch %s -> (+%o,-%o/%o) = %d \n",
            crit,plusmask,minusmask,lstatptr->st_mode &07777,
            ((newperm & 07777) == (lstatptr->st_mode & 07777)));
...
int
FilterTimeMatch(time_t stattime, char *from, char *to)
{
    time_t fromtime,totime, now = g_cfstarttime;
...
int
FilterNameRegexMatch(char *filename, char *crit)
{
    regex_t rx;
    regmatch_t pmatch;
...
int
FilterExecRegexMatch(char *filename, char *crit)
{
    regex_t rx;
    regmatch_t pmatch;
    char buffer[CF_BUFSIZE],expr[CF_BUFSIZE],line[CF_BUFSIZE],*sp;
    char ebuff[CF_EXPANDSIZE];
    FILE *pp;

    AddMacroValue(g_contextid,"this",filename);
    ExpandVarstring(crit,ebuff,NULL);
    DeleteMacro(g_contextid,"this");
...
int
FilterIsSymLinkTo(char *filename, char *crit)
{
    regex_t rx;
    regmatch_t pmatch;
    ...
    if (readlink(filename,buffer,CF_BUFSIZE-1) == -1) {
        snprintf(g_output,CF_BUFSIZE*2,
                "Unable to read link %s in filter",filename);
...
int
FilterExecMatch(char *filename, char *crit)
{
    char ebuff[CF_EXPANDSIZE];
...
void
GetProcessColumns(char *proc, char **names, int *start, int *end)
{
    char *sp,title[16];
    int col,offset = 0;
...
void
SplitLine(char *proc, char **names, int *start, int *end, char **line)
{
    int i,s,e;
    ...
    for (i = 0; names[i] != NULL; i++) {
        ...
        if (strcmp(names[i],"CMD") == 0 || strcmp(names[i],"COMMAND") == 0) {
            ...
            for (e = end[i]; (e <= end[i]+10) &&
                    !isspace((int)*(proc+e)); e++) { }
