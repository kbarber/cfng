/*
 * $Id: dce_acl.c 735 2004-05-23 06:11:20Z skaar $
 *
	    ...
	    enum fileactions   action)
{
  ...
  if (aclStatus == error_status_ok) {
	...
	if (aclStatus == error_status_ok) {
	  ...
	  /* 
       * If this is an append, copy all existing acls to the new ACL
       * list allocated by setupACL()
	   */
	  ...
	  for (ep = aces;
		...
		if (aclStatus == error_status_ok) {
		  ...
		  if (entryMatch) {
		    ...
		    /*
             * "default" means remove the named user from the ACL, so
             * that default permissions apply.  If this is a "default",
             * then decrease number of entries in the ACL; if this
             * wasn't the topmost entry, then move the topmost to this
             * slot.
		     *
             * Otherwise, set the permissions set on this entry using
             * the mode provided.
		     */
           ...
           * found a matching entry, then either this is a default
           * entry, in which case we do nothing (since there's nothing
           * to remove), or a new entry, in which case we set it up.
	       */
       ...
       * and have a new ACL, or we've been killed by a fatal error.  If
       * we are alive, and the new ACL is different from the preexisting
       * one, then we will attempt to update; otherwise, we are a no-op.
	   */
	  ...
	  if (aclStatus == error_status_ok) {
		...
		if ((action == warnall)   ||
		    (action == warnplain) ||
		    (action == warndirs)) {
		  snprintf(g_output,CF_BUFSIZE,"File %s needs ACL update\n", filename);
		  ...
		  if (aclStatus == error_status_ok) {
		    snprintf(g_output,CF_BUFSIZE,"ACL for file %s updated\n", filename);
		    CfLog(cfinform, g_output, "");
		  } else {
...
{
  /*
   * asterisk in "name" field means that the user or group id is
   * implicitly specified as the owner of the file, or that no id is
   * applicable at all (as is the case for "other").
   */
 ...
 * isMatchingACLEntry()
 *
 * Compare provided ACL entry to information contained in provided
 * rgyData structure, returning boolean value representing matched/not
 * matched status.  Status returned via status parameter.
 */
 ...
 *
 * If type information is found to be valid, return binary type.  Return
 * status via provided status parameter.
 *
			 ...
			 error_status_t * status)
{
  ...
  snprintf(g_output,CF_BUFSIZE,"Invalid DCE/DFS acl type '%s'\n", tString);
 ...
 * representation and return.  Status returned via status paramter.
 */
 ...
 *
 * Convert the provided DCE/DFS printable permission to the
 * corresponding binary representation.  Returns nonzero binary
 * representation on success, zero on failure.
 */
 ...
 * setupACL()
 *
 * dceACLData structure setup routine.  Allocate new ACL structure of
 * the requested size at aclData->new, saves old value of
 * aclData->list.sec_acls[0] at aclData->old for subsequent usage by
 * resetACL(), sets aclData->list.sec_acls[0] to the new ACL structure
 * allocated for replacement usage.
 *
 ...
 * resetACL()
 *
 * Return dceACLData structure to initial value, i.e., return the
 * original primary ACL to the appropriate location and free the
 * modified new ACL that was allocated by setupACL().
 *
		...
		error_status_t * status)
{
  ...
  /* 
   * If we've made it this far, then either the ACLs are equal in all
   * respects, or some error occurred, as indicated by the status
   * parameter.  Let the caller deal with it...
   */
